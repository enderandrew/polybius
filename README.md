...
Use `A` and `D` to move around, press `Space` to fire. There is one-time super-weapon called Superzapper - you can use it with `F`. You can jump with `W`. `Q` and `E` will dash three-lanes quickly. Arrow keys and game controllers can also be used.

On a controller, `L1`/`R1` will dash and `L2`/`R2` will lock you to a lane.
...
- All 8 phases have unique music, backgrounds and easter eggs
...
- Power-ups
  ...
  - Firewall - Lanes to the left and right of you damage Enemies
  - Phase Dash - Dash through enemies to damage them
  - Time Dilation - Time slows down for everything but you
  - 1-Up - Mario?
