  ...
  update() {
    if (this.projectileManager.rendererHelperNewProjectilesIds.length !== 0) {
      this.projectileManager.shooterProjectiles
        .filter((projectile) =>
          this.projectileManager.rendererHelperNewProjectilesIds.includes(
            projectile.objectId,
          ),
        )
        .forEach((projectile) => {
  ...
  pushProjectile(projectile) {
    if (this.projectileRenderersAvailabilityMap.length) {
      ...
      this.projectileRenderers[
        this.projectileRenderersAvailabilityMap.shift()
      ].setObjectRef(projectile);
    } else {
      // console.log(`Creating new projectile renderer #${this.projectileRenderers.length}`);
      this.projectileRenderers.push(
        new ProjectileRenderer(projectile, this.surface),
      );
