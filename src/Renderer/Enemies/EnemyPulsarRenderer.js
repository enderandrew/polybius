...
export default class EnemyPulsarRenderer extends EnemyRenderer {
  ...
  updateState() {
    ...
    if (
      (this.object.inState(EnemyPulsar.STATE_ROTATING_BEGIN) ||
        this.object.inState(EnemyPulsar.STATE_ROTATING_END)) &&
      (this.object.isFlagSet(EnemyPulsar.FLAG_ROTATION_CW) ||
        this.object.isFlagSet(EnemyPulsar.FLAG_ROTATION_CCW))
    ) {
      if (
        this.object.inState(EnemyPulsar.STATE_ROTATING_BEGIN) &&
      ...
      if (!this.isRotationStateCacheValid()) {
        this.calculateRotationStateCacheVariables(
          this.object.isFlagSet(EnemyPulsar.FLAG_ROTATION_CCW) ? 1 : -1,
        );
      }
      ...
      this.positionBase = this.surface.lanesCoords[rotationAxisLaneId]
        .clone()
  ...
  setScale() {
    ...
    if (this.object.inState(EnemyPulsar.STATE_WARNING)) {
      scale +=
        this.object.stateProgressInTime() * EnemyPulsarRenderer.PULSE_Y_SCALE;
    }

    if (this.object.inState(EnemyPulsar.STATE_PULSATING)) {
      scale +=
        (1 + Math.sin(this.object.stateProgressInTime() * Math.PI * 11.6)) *
        EnemyPulsarRenderer.PULSE_Y_SCALE;
    }
  ...
  setMaterial() {
    if (this.colorHelperPrevState !== this.object.state.id) {
      let neutral =
        !this.object.inState(EnemyPulsar.STATE_WARNING) &&
        !this.object.inState(EnemyPulsar.STATE_PULSATING);

      this.modelGroup.children[0].material = new MeshBasicMaterial({
        color: neutral
          ? EnemyPulsarRenderer.NEUTRAL_COLOR
          : EnemyPulsarRenderer.PULSE_COLOR,
      });
