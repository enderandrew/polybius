...
export default class EnemyFuseballRenderer extends EnemyRenderer {
  ...
  _applyVariantVisuals() {
    ...
    if (this._gravityOrbit)
      this._gravityOrbit.visible = !!this.object.isGravity;
    if (this._supernovaRings)
      this._supernovaRings.visible = !!this.object.isSupernova;
    if (this._voidHalo) this._voidHalo.visible = !!this.object.isVoid;

    if (this.modelGroup) {
      const s = this.object.isSupernova
        ? 1.25
        : this.object.isGravity
          ? 0.85
          : 1.0;
  ...
  updateState() {
    ...
    this.positionBase.copy(this.surface.lanesMiddleCoords[fuseball.laneId]);
    ...
    if (this._voidHalo && this._voidHalo.visible) {
      if (this.object.isPhasing) {
        ...
        this._voidHalo.children.forEach((seg) => {
          if (seg.material)
            seg.material.opacity = 0.7 + 0.3 * Math.abs(Math.sin(t / 80));
        });
        ...
        this._voidHalo.children.forEach((seg) => {
          if (seg.material)
            seg.material.opacity = 0.35 + 0.1 * Math.sin(t / 900);
        });
        if (this.modelGroup) {
          this.modelGroup.children.forEach((child) => {
  ...
  _buildVoidHalo() {
    ...
    const dashCount = 6;
    const dashSpan = Math.PI * 0.3; // Arc length of each dash in radians
    ...
    for (let i = 0; i < dashCount; i++) {
      ...
      for (let j = 0; j <= 8; j++) {
        ...
        pts.push(
          new Vector3(Math.cos(a) * orbitRadius, Math.sin(a) * orbitRadius, 0),
        );
      }
      const dash = new Line(
        new BufferGeometry().setFromPoints(pts),
