...
export class BonusStage {
  ...
  static RING_TUBE = 0.1; // Torus tube thickness
  ...
  _buildRings() {
    ...
    for (let i = 0; i < BonusStage.RING_COUNT; i++) {
      ...
      const maxStep =
        BonusStage.OFFSET_MIN_STEP +
        t * (BonusStage.OFFSET_MAX_STEP - BonusStage.OFFSET_MIN_STEP);
      const maxTilt =
        BonusStage.MAX_TILT_EARLY +
        t * (BonusStage.MAX_TILT_LATE - BonusStage.MAX_TILT_EARLY);
      ...
      const mat = new THREE.MeshBasicMaterial({
        color: BonusStage.COLOR_RING_DEFAULT,
      });
      ...
      const astGeo = new THREE.DodecahedronGeometry(
        BonusStage.ASTEROID_RADIUS,
        0,
      );
  ...
  _handleInput(delta) {
    ...
    if (this._keysDown.has('KeyA') || this._keysDown.has('ArrowLeft'))
      this.playerX += spd;
    if (this._keysDown.has('KeyD') || this._keysDown.has('ArrowRight'))
      this.playerX -= spd;
    if (this._keysDown.has('KeyW') || this._keysDown.has('ArrowUp'))
      this.playerY += spd;
    if (this._keysDown.has('KeyS') || this._keysDown.has('ArrowDown'))
      this.playerY -= spd;
  ...
  _advanceCamera(delta) {
    ...
    this.camera.position.x +=
      (this.playerX - this.camera.position.x) * 6 * delta;
    this.camera.position.y +=
      (this.playerY - this.camera.position.y) * 6 * delta;
    ...
    if (this.playerGroup) {
      ...
      this.playerGroup.rotation.z +=
        (targetBank - this.playerGroup.rotation.z) * 10 * delta;
    }
  }
  ...
  _checkRingCollisions() {
    ...
    for (let i = this.nextRingIndex; i < this.rings.length; i++) {
      ...
      if (cz >= ring.z - 0.8) {
        ...
        if (
          !ring.asteroidDestroyed &&
          dist <= BonusStage.ASTEROID_RADIUS * 1.2
        ) {
          if (this.hasShield) {
  ...
  _stageComplete() {
    ...
    window.dispatchEvent(
      new CustomEvent('bonus:complete', {
        detail: {
          totalScore: this.totalScore,
          ringsCleared: this.ringsCleared,
        },
      }),
    );
