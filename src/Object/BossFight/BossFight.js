...
const playerProjGeo = new THREE.SphereGeometry(0.35, 6, 6);
const playerProjMat = new THREE.MeshBasicMaterial({
  color: 0xffff00,
  blending: THREE.AdditiveBlending,
});

const enemyProjGeo = new THREE.IcosahedronGeometry(0.6, 0);
...
class PlayerProjectile {
  ...
  static spawn(scene, position, direction, damage = 1) {
    let proj =
      this._pool.length > 0 ? this._pool.pop() : new PlayerProjectile();
    proj.init(scene, position, direction, damage);
    return proj;
  }
  ...
  constructor() {
    ...
    this.mesh = new THREE.Mesh(playerProjGeo, playerProjMat);
  }

  init(scene, position, direction, damage) {
    this.alive = true;
    ...
    this._vel
      .copy(direction)
      .normalize()
      .multiplyScalar(PlayerProjectile.SPEED);
  ...
  dispose(scene) {
    ...
    // Push back to the pool instead of destroying geometry/material
    PlayerProjectile._pool.push(this);
  }
}
...
class EnemyProjectile {
  ...
  static spawn(scene, origin, _playerPos, speed = 9) {
    let proj = this._pool.length > 0 ? this._pool.pop() : new EnemyProjectile();
    proj.init(scene, origin, _playerPos, speed);
    return proj;
  }
  ...
  constructor() {
    this._pos = new THREE.Vector3();
    this._vel = new THREE.Vector3();
    this.mesh = new THREE.Mesh(enemyProjGeo, enemyProjMat);
  }

  init(scene, origin, _playerPos, speed) {
    this.alive = true;
    ...
    this.mesh.position.copy(this._pos);
    this.mesh.rotation.set(0, 0, 0); // Reset rotation for recycled projectiles
    scene.add(this.mesh);
  }
  ...
  dispose(scene) {
    scene.remove(this.mesh);
    // Push back to the pool instead of destroying geometry/material
    EnemyProjectile._pool.push(this);
  }
}
...
class Asteroid {
  ...
  constructor(scene, position, velocity, radius = 2.5, isGolden = false) {
    ...
    const mat = new THREE.MeshBasicMaterial({
      color: isGolden ? 0xffcc00 : 0x556677,
      wireframe: true,
    });
  ...
  break() {
    this.alive = false;
    if (this.isGolden)
      return Asteroid.TYPES[Math.floor(Math.random() * Asteroid.TYPES.length)];
...
class Minion {
  ...
  update(delta, playerPos) {
    ...
    const dir = new THREE.Vector3()
      .subVectors(playerPos, this._pos)
      .normalize();
...
class PowerToken {
  ...
  constructor(scene, position, type) {
    ...
    const mat = new THREE.MeshBasicMaterial({
      color: PowerToken.COLORS[type] ?? 0xffffff,
      blending: THREE.AdditiveBlending,
    });
    this.mesh = new THREE.Mesh(geo, mat);
    this.mesh.position.copy(this._pos);
  ...
  update(delta, playerPos) {
    ...
    if (dist < PowerToken.COLLECT_RADIUS) {
      this.alive = false;
      return true;
    }
    return false;
...
class ChaosEmerald {
  static COLORS = [
    0xff2222, 0x4444ff, 0xffee00, 0x00ee44, 0xaa00ff, 0x00ffee, 0xffffff,
  ];
  ...
  update(delta, playerPos) {
    ...
    if (this._pos.distanceTo(playerPos) < 3.5) {
      this.collected = true;
      return true;
    }
    return false;
...
export class BossFight {
  ...
  constructor(scene, camera, phaseNumber, onComplete) {
    ...
    this._difficulty =
      BossFight.PHASE_DIFFICULTY_BASE +
  ...
  _updateIntro(delta) {
    ...
    this.camera.position.set(
      0,
      0,
      THREE.MathUtils.lerp(-40, this._pz, Math.min(1, t * 1.3)),
    );
    this.camera.lookAt(0, 0, 55);
  ...
  _handleInput(delta) {
    ...
    if (
      this._shootTimer <= 0 &&
      (this._keys.has('Space') || this._keys.has('KeyZ'))
    ) {
      this._fire();
    }
  }
  ...
  _fire() {
    const cooldown =
      this._rapidFire > 0
        ? BossFight.SHOOT_COOLDOWN * 0.45
        : BossFight.SHOOT_COOLDOWN;
    ...
    angles.forEach((a) => {
      ...
      this._playerProj.push(
        PlayerProjectile.spawn(this.scene, origin, dir, damage),
      );
    });
  }
      ...
      switch (atk.type) {
        case 'projectile':
          if (atk.delay > 0) {
            this._delays.push({
              timer: atk.delay,
              fn: () => this._spawnEnemyProjectile(atk, playerPos),
            });
          } else {
        ...
        case 'minion_wave':
          for (let i = 0; i < atk.count; i++) {
            this._delays.push({
              timer: i * 0.35,
              fn: () => this._minions.push(new Minion(this.scene, atk.origin)),
            });
          }
          break;
        ...
        case 'boss_dead':
          this._emerald = new ChaosEmerald(
            this.scene,
  ...
  _spawnEnemyProjectile(atk, _playerPos) {
    ...
    this._enemyProj.push(
      EnemyProjectile.spawn(this.scene, atk.origin, target, atk.speed ?? 9),
    );
  }
  ...
  _spawnAsteroidWave(count, phase) {
    ...
    for (
      let i = 0;
      i < scaledCount && this._asteroids.length < BossFight.MAX_ASTEROIDS;
      i++
    ) {
  ...
  _tickEntities(delta, playerPos) {
    ...
    this._playerProj = this._playerProj.filter((p) => {
      if (!p.alive) {
    ...
    this._enemyProj = this._enemyProj.filter((p) => {
      if (!p.alive) {
        p.dispose(this.scene);
        return false;
      }
    ...
    this._eyeBeams = this._eyeBeams.filter((b) => {
      if (!b.alive) {
    ...
    this._minions.forEach((m) => {
      if (m.update(delta, playerPos)) {
        this._enemyProj.push(
          EnemyProjectile.spawn(this.scene, m.position.clone(), playerPos, 7),
        );
      }
    });
    this._minions = this._minions.filter((m) => {
      if (!m.alive) {
    ...
    this._tokens.forEach((t) => {
      if (t.update(delta, playerPos)) this._applyPowerUp(t.type);
    });
  ...
  _checkCollisions(playerPos) {
    ...
    this._playerProj.forEach((proj) => {
      ...
      eyePositions.forEach((ep) => {
        if (
          proj.position.distanceTo(ep) <
          BossEntity.EYE_RADIUS + proj.radius
        ) {
          proj.alive = false;
          this._score += 50;
      ...
      this._asteroids.forEach((ast) => {
        ...
        if (proj.position.distanceTo(ast.position) < ast.radius + proj.radius) {
          ...
          if (drop)
            this._tokens.push(
              new PowerToken(this.scene, ast.position.clone(), drop),
            );
        }
      });
      // vs minions
    ...
    this._enemyProj.forEach((proj) => {
      if (
        proj.alive &&
        proj.position.distanceTo(playerPos) < 1.2 + proj.radius
      ) {
        proj.alive = false;
        this._hitPlayer();
      }
    ...
    // Eye beams vs player
    this._eyeBeams.forEach((b) => {
      if (b.hitsPlayer(playerPos)) this._hitPlayer();
    });
  ...
  _applyPowerUp(type) {
    ...
    switch (type) {
      case 'HEALTH':
        this._playerHP = Math.min(BossFight.PLAYER_MAX_HP, this._playerHP + 1);
        break;
      case 'SHIELD':
        this._shield = 3.0;
        break;
      case 'MULTI':
        this._multiShot = 10.0;
        break;
      case 'RAPID':
        this._rapidFire = 10.0;
        break;
      case 'POWER':
        this._powerBoost = 12.0;
        break;
    }
  ...
  _end(victory) {
    ...
    setTimeout(
      () => {
        if (this.onComplete) this.onComplete(victory, this._score);
      },
      victory ? 500 : 1500,
    );
  }
  ...
  _buildHUD() {
    ...
    this._hud.style.cssText =
      'position:absolute;top:0;left:0;right:0;z-index:600;pointer-events:none;font-family:"Courier New",monospace';

    // Boss HP bar
    const bossRow = document.createElement('div');
    bossRow.style.cssText =
      'display:flex;align-items:center;gap:8px;justify-content:center;padding:10px 0 4px';
    this._bossLabel = document.createElement('span');
    this._bossLabel.style.cssText =
      'color:#ff2200;font-size:13px;letter-spacing:0.1em;text-shadow:0 0 8px #ff2200;white-space:nowrap';
    this._bossLabel.textContent = 'SYNTHETIC OVERLORD';
    const bossBarWrap = document.createElement('div');
    bossBarWrap.style.cssText =
      'width:300px;height:10px;background:#1a0000;border:1px solid #ff2200;border-radius:2px;overflow:hidden;flex-shrink:0';
    this._bossBar = document.createElement('div');
    this._bossBar.style.cssText =
      'height:100%;background:#ff2200;box-shadow:0 0 6px #ff2200;transition:width 0.2s';
    ...
    this._hpDisplay.style.cssText =
      'color:#00ff44;font-size:18px;text-shadow:0 0 8px #00ff44';
    this._scoreDisplay = document.createElement('div');
    this._scoreDisplay.style.cssText =
      'color:#ffffff;font-size:13px;letter-spacing:0.1em';
    this._puDisplay = document.createElement('div');
    this._puDisplay.style.cssText =
      'color:#ffaa00;font-size:12px;letter-spacing:0.08em;text-align:right;text-shadow:0 0 6px #ffaa00';
    ...
    this._tauntEl.style.cssText = [
      'position:absolute',
      'top:15%', // Dropped down from 58px to sit closer to the action
      'left:50%',
      'transform:translateX(-50%)',
      'color:#ffcccc', // Lightened the core text color slightly to contrast with the red glow
      'font-size:24px', // Bumped up significantly from 14px
      'font-weight:bold', // Added weight for readability
      'letter-spacing:0.12em',
      'text-align:center',
      'text-shadow: 0 0 15px #ff2200, 0 0 5px #ff2200', // Stacked shadows for a stronger neon CRT glow
      'opacity:0',
  ...
  _updateHUD() {
    if (!this._hud) return;
    this._bossBar.style.width =
      Math.max(0, (this.boss.hp / BossEntity.MAX_HP) * 100) + '%';

    const full = '♥'.repeat(this._playerHP);
    const empty = '♡'.repeat(
      Math.max(0, BossFight.PLAYER_MAX_HP - this._playerHP),
    );
  ...
  _showTaunt(text) {
    ...
    this._tauntTimeout = setTimeout(() => {
      if (this._tauntEl) this._tauntEl.style.opacity = '0';
    }, 2800);
  }
  ...
  _updateShip(delta) {
    ...
    const worldPos = this._shipLocalOffset
      .clone()
    ...
    this._shipRoll = THREE.MathUtils.lerp(
      this._shipRoll,
      targetRoll,
      delta * 6,
    );
    this._shipPitch = THREE.MathUtils.lerp(
      this._shipPitch,
      targetPitch,
      delta * 6,
    );
    ...
    // Engine pulse
    if (this._shipEngine) {
      this._shipEngine.material.opacity =
        0.6 + 0.4 * Math.abs(Math.sin(performance.now() / 150));
    }

    // Flicker on hit
    this._shipGroup.visible =
      this._invincible <= 0 || Math.floor(performance.now() / 80) % 2 === 0;
