...
export class BossEntity {
  ...
  static PHASE_THRESHOLDS  = [0.70, 0.40];
  ...
  update (delta, playerPos) {
    ...
    if (this.isDying) { this._updateDeathAnimation(delta); return null; }
  ...
  takeDamage (damage) {
    if (!this.isAlive || this.isDying) return { phaseBroke: false, taunt: null };
    ...
    if (newPhase > this.phase) {
      ...
      return {
        phaseBroke: true,
        taunt: BossEntity.TAUNTS[Math.floor(Math.random() * BossEntity.TAUNTS.length)],
      };
    }
  ...
  getEyeWorldPositions () {
    return this._eyeSpheres.map(sphere => {
  ...
  _buildHead () {
    ...
    this._headWire = new THREE.LineSegments(wireGeo,
      new THREE.LineBasicMaterial({ color: 0xcc1100, transparent: true, opacity: 0.7 })
    );
    ...
    this._innerWire = new THREE.LineSegments(innerWire,
      new THREE.LineBasicMaterial({ color: 0x661100, transparent: true, opacity: 0.4 })
    );
  ...
  _buildHorns () {
    ...
    const hornMat = new THREE.MeshBasicMaterial({ color: 0xff2200, wireframe: true });
  ...
  _buildEyes () {
    ...
    [
      ...
      const socket    = new THREE.Mesh(socketGeo,
  ...
  _buildOrbit () {
    ...
    const mat = new THREE.LineBasicMaterial({ color: 0x880000, transparent: true, opacity: 0.55 });
    ...
    for (let i = 0; i < 8; i++) {
      ...
      mesh.userData.speed = 0.12 + Math.random() * 0.10;
  ...
  _animateIdle (delta, playerPos) {
    ...
    // Vertical bob
    this._group.position.y = this.position.y + Math.sin(this._bobTime * 0.7) * 1.2;

    // Subtle tracking tilt toward player
    const tiltX = THREE.MathUtils.clamp((playerPos.y - this._group.position.y) * -0.02, -0.15, 0.15);
    const tiltY = THREE.MathUtils.clamp((playerPos.x - this._group.position.x) *  0.03, -0.18, 0.18);
    this._group.rotation.x = THREE.MathUtils.lerp(this._group.rotation.x, tiltX, delta * 2);
    this._group.rotation.y = THREE.MathUtils.lerp(this._group.rotation.y, tiltY, delta * 2);
    ...
    // Mouth open/close
    if (this._mouthDir !== 0) {
      this._mouthOpen = THREE.MathUtils.clamp(this._mouthOpen + delta * 2.5 * this._mouthDir, 0, 1);
    ...
    if (this._chargeProgress > 0) {
      ...
      const lunge = Math.sin(Math.PI * (1 - this._chargeProgress)) * 12 * this._chargeDir;
  ...
  _selectAttack () {
    const r = Math.random();
    if (this.phase === 0) return r < 0.5 ? BossEntity.ATK_PROJECTILE_BURST : BossEntity.ATK_ASTEROID_WAVE;
    if (this.phase === 1) {
      if (r < 0.30) return BossEntity.ATK_PROJECTILE_BURST;
    ...
    if (r < 0.60) return BossEntity.ATK_CHARGE;
  ...
  _fireAttack (type, playerPos) {
    ...
    switch (type) {
      ...
      case BossEntity.ATK_PROJECTILE_BURST: {
        ...
        this._mouthDir = 1;
        setTimeout(() => { this._mouthDir = -1; }, 1500);
        break;
      }
      ...
      case BossEntity.ATK_ASTEROID_WAVE:
        this.pendingAttacks.push({ type: 'asteroid_wave', count: 6 + this.phase * 3, phase: this.phase });
        break;

      case BossEntity.ATK_MINION_WAVE:
        this.pendingAttacks.push({ type: 'minion_wave', count: 2 + this.phase, origin: this.getMouthWorldPosition() });
        this._mouthDir = 1;
        setTimeout(() => { this._mouthDir = -1; }, 2200);
        break;
  ...
  _updatePhase () {
    ...
    if (p > this.phase) { this.phase = p; this._onPhaseEnter(p); }
  }
  ...
  _onPhaseEnter (phase) {
    ...
    this.pendingAttacks.push({ type: 'asteroid_wave', count: 8 + phase * 4, phase });
  }
  ...
  _updateDeathAnimation (delta) {
    ...
    this._eyeMaterials.forEach(mat => { if (mat) mat.color.setRGB(1, pulse, pulse); });
