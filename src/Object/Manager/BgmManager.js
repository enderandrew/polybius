export default class BgmManager {
  ...
  playForLevel(level) {
    ...
    this.audio.play().catch(err => console.warn('BGM Play Prevented by Browser:', err));
  ...
  update() {
    ...
    const nextAccent = this.beatData.accents.find(t => t > this.lastTime && t <= currentTime);
    if (nextAccent) {
      this.onAccent();
    } else {
      // Only trigger a normal beat if it wasn't already a heavy accent
      const nextBeat = this.beatData.beats.find(t => t > this.lastTime && t <= currentTime);
