 ...
 *
 * ── Resolution reverted to 1024×1024 ──────────────────────────────────────
 * A prior version dropped this to 512×512 to save texture upload bandwidth,
 * but that broke on-screen scaling — content rendered tiny and pinned to a
 * corner instead of filling the CRT plane, which every other 1024-resolution
 * screen in the game (ScreenPlay, ScreenParodySurface, etc.) does correctly.
 * Whatever relationship Canvas3d/the screen plane has with its expected
 * resolution, 512 broke it. Reverted to 1024 to match every other screen;
 * the static pre-render caching still delivers the bulk of the performance
 * win without touching that relationship.
 *
 * Story-page text has also been enlarged and spread out further than the
 * original 1024 version — it was rendering correctly but felt small/sparse
...
export default class ScreenAttractMode extends Canvas3d {
  ...
  static PAGES = [
    { id: 'story1',      duration: 5000 },
    { id: 'story2',      duration: 5000 },
    { id: 'story3',      duration: 5000 },
  ...
  static ENEMY_GALLERY = [
    ...
    { level:  3, asset: 'flipperTanker',  color: '#ffaa00', name: 'FLIPPER TANKER' },
    { level:  5, asset: 'spiker',         color: '#00ff00', name: 'SPIKER' },
    { level:  7, asset: 'mutant_flipper', color: '#ff2222', name: 'MUTANT FLIPPER',
    ...
    { level: 14, asset: 'stealth_flipper',color: '#aa44ff', name: 'STEALTH FLIPPER',
      opacity: 0.42, filter: 'hue-rotate(65deg) saturate(1.3) brightness(0.95)' },
    { level: 17, asset: 'pulsar',         color: '#00ffff', name: 'PULSAR' },
    { level: 20, asset: 'fuseball',       color: '#ff4400', name: 'GRAVITY FUSEBALL',
      filter: 'hue-rotate(-43deg) saturate(1.5) brightness(1.1)', effect: 'gravity-orbit' },
    { level: 23, asset: 'spiker',         color: '#44ff88', name: 'PHANTOM SPIKER', opacity: 0.38 },
    { level: 26, asset: 'fuseball',       color: '#88ddff', name: 'SUPERNOVA FUSEBALL',
      filter: 'hue-rotate(36deg) saturate(1.3) brightness(1.05)', effect: 'supernova-rings' },
    { level: 29, asset: 'pulsar',         color: '#ff4488', name: 'MEGA PULSAR',
      filter: 'hue-rotate(-18deg) saturate(1.6) brightness(1.1)', effect: 'mega-brackets' },
    { level: 32, asset: 'fuseball',       color: '#aa44ff', name: 'VOID FUSEBALL',
      filter: 'hue-rotate(79deg) saturate(1.4) brightness(0.9)', effect: 'void-halo' },
    { level: 35, asset: 'fuseballTanker', color: '#ff8800', name: 'FUSEBALL TANKER' },
    { level: 38, asset: 'pulsar',         color: '#ff8844', name: 'INVERSE PULSAR',
      filter: 'hue-rotate(30deg) saturate(1.4) brightness(1.15)' },
    { level: 41, asset: 'pulsarTanker',   color: '#00ffcc', name: 'PULSAR TANKER' },
    { level: 44, asset: 'spiker',         color: '#ffaa00', name: 'HYDRA SPIKER',
      filter: 'hue-rotate(-29deg) saturate(1.6) brightness(1.15)', effect: 'hydra-fork' },
    { level: 48, asset: 'pulsar',         color: '#ff44ff', name: 'CHAOS PULSAR',
      filter: 'hue-rotate(140deg) saturate(2.0) brightness(1.1)' },
    { level: 52, asset: 'spiker',         color: '#88eeff', name: 'OVERDRIVE SPIKER',
      filter: 'hue-rotate(54deg) saturate(1.5) brightness(1.4)', effect: 'overdrive-streaks' },
    { level: 56, asset: 'flipperTanker',  color: '#cc88ff', name: 'PHANTOM TANKER',
      opacity: 0.55, filter: 'hue-rotate(65deg) saturate(1.3) brightness(0.95)' },
    { level: 60, asset: 'mirror',         color: '#dddddd', name: 'MIRROR' },
    { level: 64, asset: 'flipperTanker',  color: '#ff6622', name: 'BOMB TANKER',
      filter: 'hue-rotate(-22deg) saturate(1.7) brightness(1.15)', effect: 'bomb-pulse' },
  ];
  ...
  constructor (screenContentManager, highScores, onExit) {
    ...
    if (typeof onExit !== 'function') {
      console.error('ScreenAttractMode: onExit callback missing. Check the constructor call site.');
    }
  ...
  update () {
    ...
    if (Math.random() < 0.10) this._spawnGlitch(now);
  ...
  _prerenderStaticPages () {
    ScreenAttractMode.PAGES.forEach(({ id }) => {
      ...
      switch (id) {
        case 'story1':     this._drawStory1Static(ctx);     break;
        case 'story2':     this._drawStory2Static(ctx);     break;
        case 'story3':     this._drawStory3Static(ctx);     break;
        case 'enemies':    this._drawEnemiesStatic(ctx);    break;
        case 'highscores': this._drawHighScoresStatic(ctx); break;
      }
  ...
  _drawStory1Static (ctx) {
    ...
    this._t(ctx, 'OPERATION: SYNAPSE SNAP', cx, y, '#00ffff', 58);
    y += 20; this._rule(ctx, cx, y, 440, '#00ffff'); y += 70;

    this._t(ctx, 'IN 1981, THE GOVERNMENT DEPLOYED AN ARCADE GAME.', cx, y, '#ffffff', 30);
    y += 54;
    this._t(ctx, 'THEY SAID IT WAS JUST FOR ENTERTAINMENT.', cx, y, '#ffffff', 30);
    y += 96;

    this._t(ctx, 'THEY LIED.', cx, y, '#ff2222', 74);
    y += 118;

    this._t(ctx, 'IT WAS A NEURAL CALIBRATION TOOL.', cx, y, '#00ff44', 32);
    y += 56;
    this._t(ctx, 'DESIGNED TO HARVEST PSYCHIC ENERGY.', cx, y, '#00ff44', 32);
  }
  ...
  _drawStory2Static (ctx) {
    ...
    this._t(ctx, 'YOUR MISSION:', cx, y, '#00ffff', 58);
    y += 20; this._rule(ctx, cx, y, 440, '#00ffff'); y += 70;

    this._t(ctx, 'INFILTRATE THE MAINFRAME.', cx, y, '#ffffff', 30);
    y += 54;
    this._t(ctx, 'DESCEND THROUGH 256 SECTORS OF GEOMETRIC MADNESS.', cx, y, '#ffffff', 30);
    y += 92;

    this._t(ctx, 'RECOVER THE 7 CLASSIFIED "CHAOS EMERALDS".', cx, y, '#ffff00', 36);
    y += 66;

    this._t(ctx, '(DO NOT ASK WHY THE CIA WANTS THEM.)', cx, y, '#666666', 22, false);
  }
  ...
  _drawStory3Static (ctx) {
    ...
    this._t(ctx, 'THE SIMULATION HAS 8 PHASES.', cx, y, '#00ffff', 58);
    y += 20; this._rule(ctx, cx, y, 440, '#00ffff'); y += 70;

    this._t(ctx, 'COLLECT THE EMERALDS.', cx, y, '#ffffff', 30); y += 54;
    this._t(ctx, 'DEFEAT THE PULSARS.',   cx, y, '#ffffff', 30); y += 54;
    this._t(ctx, 'REACH THE RAINBOW PHASE TO WAKE UP.', cx, y, '#ffffff', 30);

    this._story3FlashY = y + 110;
  }
  ...
  _drawStory3Dynamic (ctx) {
    ...
    ctx.shadowBlur  = 26;
    this._t(ctx, 'INSERT COIN TO SUBMIT YOUR WILL.', cx, this._story3FlashY, '#ff2222', 44);
    ctx.shadowBlur  = 0;
  }
  ...
  _drawEnemiesStatic (ctx) {
    ...
    gallery.forEach((entry, idx) => {
      ...
      if (entry.opacity !== undefined)   ctx.globalAlpha = entry.opacity;
  ...
  _drawEnemyIcon (ctx, asset, cx, cy, size) {
    ...
    asset.coords.forEach(group => group.forEach(({ x, y }) => {
    ...
    asset.coords.forEach((group, i) => {
      ...
      if (raw !== undefined) {
        const n = typeof raw === 'number' ? raw : parseInt(String(raw).replace('#', ''), 16);
    ...
    switch (effect) {
      ...
      case 'void-halo': {
        ...
        for (let i = 0; i < 6; i++) {
          ...
          ctx.beginPath();
          ctx.arc(cx, cy, r * 1.25, center - dashSpan / 2, center + dashSpan / 2);
          ctx.stroke();
        }
        break;
      }
      case 'mega-brackets': {
        ...
        ctx.lineWidth   = 1.5;
        const h = r * 0.9, armW = r * 0.35, pad = r * 1.4;
        [[-1, pad], [1, -pad]].forEach(([dir, xOff]) => {
          ctx.beginPath();
  ...
  _drawHighScoresStatic (ctx) {
    ...
    if (scores.length === 0) {
      ...
      this._t(ctx, 'THE SIMULATION IS WATCHING.', cx, 400, '#444444', 20, false);
    } else {
    ...
    ctx.textAlign = 'center';
    this._t(ctx, 'ALL SCORES ARE FORWARDED TO A FACILITY IN UTAH.', cx, 848, '#2a2a2a', 14, false);
