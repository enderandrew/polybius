...
export default class ScreenHighScores extends Canvas3d {
  ...
  constructor(
    ...
    this.highScores = [
      ...this.screenContentManager.get(ScreenContentManager.KEY_HIGH_SCORES),
    ];
  ...
  step() {
    ...
    if (this.currentStep >= 3) {
      ...
      )(this.score, this.playerName.join(''));
      this.screenContentManager.get(
  ...
  draw() {
    ...
    for (let i = 0; i < this.highScores.length; i++) {
      ...
      if (Math.random() > 0.995) {
        ...
        displayColor = Canvas3d.COLOR_RED;
      }

      this.drawText(
        this.alignNumberToRight(this.highScores[i].score),
        550,
        340 + i * 50,
        displayColor,
      );
