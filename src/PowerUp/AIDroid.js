...
export class AIDroid {
  ...
  static FLOAT_HEIGHT = 0.7; // Units above the player's rim Y position
  ...
  _buildMesh() {
    ...
    faceConfigs.forEach(({ pos, euler }) => {
      ...
      face.add(this._circle(s * 0.2, 0xff88cc));
  ...
  _circle(radius, color) {
    ...
    for (let i = 0; i <= segs; i++) {
      ...
      pts.push(
        new THREE.Vector3(Math.cos(a) * radius, Math.sin(a) * radius, 0.002),
      );
    }
    return new THREE.Line(
      new THREE.BufferGeometry().setFromPoints(pts),
